...
在 `spreadsheet_page` 增加二维码扫码入口。扫码到设备编码后定位现有设备行并进入统一的设备编辑页；新增操作也进入该编辑页。编辑页保存成功后立即通过现有回调更新外部列表。

## Scope

- 使用 `mobile_scanner` 打开相机并扫码二维码。
...
只负责相机预览和扫码结果。使用 `mobile_scanner` 的检测回调，在首个有效值到达后停止扫码并返回设备编码。权限拒绝、初始化失败和无法使用相机时显示提示并允许返回。
...
- 相机权限被拒绝或扫码初始化失败时提示错误并允许返回。
- 保存失败时保留编辑页输入，允许重试；不会调用成功后的页面返回逻辑。
- 扫码成功后立即停止扫码，避免同一二维码重复触发。
